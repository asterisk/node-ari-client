...
function Client(baseUrl, user, pass) {
  ...
  //     'cleanupEventTypes': ['eventType',...]
  //   },...],
  // }
  self._instanceListeners = {};

  // Keep track of instances that are managed by the client in order to cleanup
  // event listeners after a cleanup event is fired for that instance
  // {
  //   'id': uniqueId
  // }
  self._managedInstances = {};
}
...
Client.prototype.start = function (apps, subscribeAll, callback) {
  ...
  return new Promise(function(resolve, reject) {
    ...
    function processMessage (msg, flags) {
      ...
      var resources = {};
      // IDs of resources included in this event
      var resourceIds = [];
      // IDs of instances with event listeners for this event
      var instanceIds = [];
      ...
      // remove managed listeners from the client in the event of a cleanup
      // event being fired
      _.each(Object.keys(self._instanceListeners), function (eventType) {
        var listeners = self._instanceListeners[eventType];
        var managedListeners = _.filter(listeners, function (listener) {
          return listener.isManaged &&
                 _.contains(listener.cleanupEventTypes, event.type) &&
                 _.contains(resourceIds, listener.id);
        });
        var updatedListeners = _.filter(listeners, function (listener) {
          return !listener.isManaged ||
                 !_.contains(listener.cleanupEventTypes, event.type) ||
                 !_.contains(resourceIds, listener.id);
        });

        self._instanceListeners[eventType] = updatedListeners;

        _.each(managedListeners, function (listener) {
          self.removeListener(
            util.format('%s-%s', eventType, listener.id),
            listener.callback
          );
          delete self._managedInstances[listener.id];
        });
      });
    }
