 ...
 *  @prop {Object} _instanceListeners - array of instance identifiers for
...
function Client(baseUrl, user, pass) {
  ...
  // Keep track of instance event listeners
  //
  // {
  //   'eventType': [uniqueid, uniqueid...],
  //   ...
  // }
  ...
  function processMessage (msg, flags) {
    ...
    _.each(eventModel.properties, function (prop) {
      if (_.contains(_resources.knownTypes, prop.dataType) &&
        ...
        if (listeners &&
            _.contains(listeners, instanceId) &&
            !_.contains(instanceIds, instanceId)) {
