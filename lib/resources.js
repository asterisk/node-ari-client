...
function Resource (client, id, objValues) {
  ...
  // is this a managed instance?
  if (self._client._managedInstances[self._id()]) {
    self._isManaged = true;
  }
}

/**
 *  Mark this instance as managed so event listeners can be automatically
 *  cleaned up when the resource is destroyed.
 *
 *  @method manageInstance
 *  @memberof module:resources~Resource
 */
Resource.prototype.manageInstance = function () {
  var self = this;

  self._isManaged = true;
  self._client._managedInstances[self._id()] = self._id();
};
...
Resource.prototype.on = function (event,
  ...
  listeners[event].push({
    ...
    cleanupEventTypes: self._cleanupEventTypes
  });
...
/**
 *  List of events for which a bridge should be cleaned up.
 *
 *  @member {string} _cleanupEventTypes
 *  @memberof module:resources~Bridge
 */
Bridge.prototype._cleanupEventTypes = ['BridgeDestroyed'];
...
/**
 *  List of events for which a channel should be cleaned up.
 *
 *  @member {string} _cleanupEventTypes
 *  @memberof module:resources~Channel
 */
Channel.prototype._cleanupEventTypes = ['StasisEnd'];
...
/**
 *  List of events for which a live recording should be cleaned up.
 *
 *  @member {string} _cleanupEventTypes
 *  @memberof module:resources~LiveRecording
 */
LiveRecording.prototype._cleanupEventTypes = [
  'RecordingFinished',
  'RecordingFailed'
];
...
/**
 *  List of events for which a playback should be cleaned up.
 *
 *  @member {string} _cleanupEventTypes
 *  @memberof module:resources~Playback
 */
Playback.prototype._cleanupEventTypes = ['PlaybackFinished'];
