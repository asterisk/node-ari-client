...
Resource.prototype.on = function (event,
  ...
  if (!_.contains(listeners[event], id)) {
    listeners[event].push(id);
  }
...
Resource.prototype.removeListener = function (event, callback) {
  ...
  if (listeners[event]) {
    var scopedEvent = util.format('%s-%s', event, id);
    var index = _.indexOf(listeners[event], id);

    if (index > -1) {
      // if this is the last scoped event for this resource, remove it from
      // the list of instance listeners
      if (self._client.listeners(scopedEvent).length === 1) {
        listeners[event].splice(index, 1);
      }
      self._client.removeListener(scopedEvent, callback);
    }
  }
};

/**
 *  Unattached all events from this resource.
 *
 *  @method removeListener
 *  @memberof module:resources:~Resource
 *  @param {string} event - event name
 */
Resource.prototype.removeAllListener = function (event) {
  var self = this;
  var id = self._id() && self._id().toString();
  var listeners = self._client._instanceListeners;
